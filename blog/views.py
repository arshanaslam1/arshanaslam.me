...
class PostList(ListView):
    ...
    # ordering = ['-updated_on']
    queryset = Post.objects.published().approved().filter(author=1).order_by('-created_on')
    paginate_by = 7
...
class PostDetail(HitCountDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        related_posts = Post.objects.published().approved().filter(author=1,
...
class TagPostView(ListView):
    ...
    def get_queryset(self):
        ...
        results_filter = Post.objects.approved().published().filter(author=1,tags=self.tag).order_by('-created_on')
...
class CategoryPostListView(ListView):
    ...
    def get_queryset(self):
        ...
        results_filter = Post.objects.approved().published().filter(author=1,
    ...
    def get_queryset(self):
        ...
        try:
            search_posts = Post.objects.approved().published().filter(
                Q(title__icontains=self.query),author=1
